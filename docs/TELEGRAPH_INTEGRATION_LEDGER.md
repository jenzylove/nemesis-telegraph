...
| Settled x402 payment | Prove paid Telegraph usage | Yes | Credential needed | Needs explicit payment approval and a funded burner key. No payment was attempted. |
| Routed miner response | Validate selected miner, intent, result, latency | Yes | Credential needed | Blocked on settled payment. Current OpenAPI promises top-two primary/fallback. |
| Router probabilistic behavior | Support quality-flywheel claim | Track 3 important | Blocked | Rules describe probabilistic routing; current API docs describe top-two primary/fallback. Ask Telegraph/Discord and observe paid calls. |
| Payment receipt metadata | Audit spend/settlement | Yes | Credential needed | Need real `PAYMENT-RESPONSE` from paid replay. |
| Signal hash / verification metadata | Proof in UI/audit | Desired | Blocked | Not guaranteed by HTTP response schema. Explorer has signal hashes; determine linkage after paid call. |
...
| Secret Manager burner key | Keep payer key out of API/frontend | Yes | Credential needed | Create only for implementation; Base Sepolia path recommended. |
