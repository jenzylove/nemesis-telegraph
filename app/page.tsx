...
import ProvisionalIncident from "./provisional-incident";
...
type IncidentDiscovery={source:"bitquery"|"alchemy";status:"SELECTED"|"PROVISIONAL_INCIDENT"|"AMBIGUOUS_INCIDENT";selected_transaction_hash:string|null;selected_score:number|null;incident_selection_confidence:number;ambiguity_reason:string|null;candidate_count:number;incident_time:string|null;candidates:DiscoveryCandidate[]};
...
type CaseSection="overview"|"graph"|"evidence"|"intelligence"|"assessment"|"timeline";
...
/**
 * Wei is precise and unreadable. The rounded figure is for the person whose
 * money this is; the exact integer stays beside it for anyone checking.
 */
function amount(raw:number|string,asset:string){
 const n=typeof raw==="number"?raw:Number(raw);
 if(!Number.isFinite(n))return String(raw);
 if(asset!=="native")return n.toLocaleString("en-US");
 const eth=n/1e18;
...
function Shell({children,onExit,onHome,monitor=false}:{children:(section:CaseSection,go:(next:CaseSection)=>void)=>React.ReactNode;onExit:()=>void;onHome:()=>void;monitor?:boolean}){
 ...
 return <main className="casePage"><aside><Brand onClick={onHome}/><div className="caseNav"><span>CASE</span>
  ...
  <button className={section==="graph"?"selected":""} onClick={()=>setSection("graph")}>⌁ Fund trace</button>
  <button className={section==="evidence"?"selected":""} onClick={()=>setSection("evidence")}>◎ Verified evidence</button>
  <button className={section==="intelligence"?"selected":""} onClick={()=>setSection("intelligence")}>◇ Telegraph intelligence</button>
  <button className={section==="assessment"?"selected":""} onClick={()=>setSection("assessment")}>◆ Assessment</button>
  <button className={section==="timeline"?"selected":""} onClick={()=>setSection("timeline")}>▤ Timeline</button>
 </div>{monitor&&<div className="monitorBox"><i/><div><b>MONITOR ACTIVE</b><small>1 dormant branch</small></div></div>}<button className="newCase" onClick={onExit}>＋ New investigation</button></aside>{children(section,setSection)}</main>
}
...
function RealCaseScreen({data,onExit,onHome,isPublic=false,onReplace}:{data:RealResponse;onExit:()=>void;onHome:()=>void;isPublic?:boolean;onReplace?:(next:RealResponse)=>void}){
 const c=data.case,tx=c.evidence!.transaction,f=c.finding||{classification:"unknown",summary:"",confidence:0,compromise_mechanism_confidence:null,evidence_references:[],limitations:[]};
 // An assessment that could not be produced is still an honest answer, and it
 // is phrased for the person reading it rather than for whoever configured it.
 const undetermined=!c.finding||f.classification==="unknown"||f.classification==="agent unavailable";
 ...
 // Overriding a provisional pick opens a fresh investigation on the chosen
 // transaction. It is the same explicit-selection path as before, reachable
 // now without ever having blocked the case.
 const[overrideBusy,setOverrideBusy]=useState<string|null>(null);const[overrideError,setOverrideError]=useState("");
 const chooseAlternative=async(hash:string)=>{
  if(overrideBusy)return;setOverrideBusy(hash);setOverrideError("");
  const api=process.env.NEXT_PUBLIC_NEMESIS_API_URL?.replace(/\/$/,"");
  if(!api){setOverrideError("Investigation API is not configured.");setOverrideBusy(null);return;}
  const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),180000);
  try{
   const r=await fetch(`${api}/v1/cases`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({wallet_address:c.wallet_address,chain:c.chain,theft_transaction_hash:hash}),signal:controller.signal});
   const payload=await r.json();
   if(!r.ok)throw new Error(payload.detail||"Candidate investigation failed");
   onReplace?.(payload as RealResponse);
  }catch(reason){setOverrideError(reason instanceof Error?reason.message:"Candidate investigation failed")}
  finally{clearTimeout(timeout);setOverrideBusy(null)}
 };
 ...
 const lifecycle=[
  {label:"Incident identified",detail:"Verified onchain",done:true,tone:"chain"},
  ...
  {label:"Funds traced",detail:"Movement verified onchain",done:timelineTypes.has("MOVEMENT_DETECTED")||timelineTypes.has("TRACING_RESUMED"),tone:"chain"},
  {label:"Telegraph intelligence purchased",detail:"External context",done:timelineTypes.has("TELEGRAPH_ENRICHMENT_REQUESTED")||successfulTelegraph.length>0,tone:"telegraph"},
  {label:"Monitoring active",detail:successfulTelegraph.length?"Proof stored":"Awaiting movement",done:successfulTelegraph.length>0,tone:"telegraph"},
 ];
 ...
 return <Shell onExit={onExit} onHome={onHome} monitor={!!trace?.branches.some(b=>b.status==="DORMANT")}>{(section,go)=><section className="caseMain"><header><div><div className="eyebrow">{isPublic?"PUBLIC DEMO · READ ONLY":"CASE"} {c.id}</div><h2>Incident investigation</h2><div className={`status ${tx.status==="success"?"actionable":"dormant"}`}><i/>{trace?.case_state||c.state}</div></div></header><div className="metrics"><div><small>CHAIN</small><strong>{c.chain.toUpperCase()}</strong><span>Onchain verified</span></div><div><small>TRANSACTION STATUS</small><strong className="accent">{tx.status.toUpperCase()}</strong><span>{c.discovery?"Found from wallet history · verified onchain":"Verified onchain"}</span></div><div><small>TRACE BRANCHES</small><strong>{trace?.branches.length||0}</strong><span>{trace?.branches.filter(b=>b.status==="DORMANT").length||0} monitored</span></div><div><small>TELEGRAPH INTELLIGENCE</small><strong>{telegraph?.summary?.accepted??0}</strong><span>accepted of {telegraph?.summary?.responses_purchased??0} purchased</span></div></div>{section==="overview"&&<CaseOverview caseState={trace?.case_state||c.state} chain={c.chain} txStatus={tx.status} txHash={tx.hash} branchCount={trace?.branches.length||0} dormantCount={trace?.branches.filter(b=>b.status==="DORMANT").length||0} telegraphPaid={telegraph?.summary?.responses_purchased??successfulTelegraph.length} telegraphAccepted={telegraph?.summary?.accepted||0} telegraphNoSignal={telegraph?.summary?.no_case_signal||0} telegraphConflicted={telegraph?.summary?.conflicted||0} telegraphSpend={telegraph?.summary?.spend_usd||0} minerNames={telegraph?.summary?.miners||[]} provisional={c.discovery?.status==="PROVISIONAL_INCIDENT"} confidence={c.discovery?.incident_selection_confidence||0} lifecycle={lifecycle} go={go}/>}{c.discovery?.status==="PROVISIONAL_INCIDENT"&&(section==="overview"||section==="evidence")&&<ProvisionalIncident confidence={c.discovery.incident_selection_confidence} reason={c.discovery.ambiguity_reason} selectedHash={c.discovery.selected_transaction_hash} candidates={c.discovery.candidates} onChoose={isPublic?()=>{}:chooseAlternative} busyHash={overrideBusy} error={overrideError}/>}<div className="workspace" style={section==="overview"?undefined:{gridTemplateColumns:"1fr"}}>
  {section==="graph"&&<section className="panel graphPanel"><div className="panelHead"><div><span>LIVE FUND GRAPH</span><small>{trace?.branches.some(b=>b.status==="MOVING")?" TRACING IN PROGRESS":" VERIFIED ONCHAIN"}</small></div></div><RealGraph data={data} trace={trace}/></section>}
  {section==="timeline"&&<section className="panel activity realActivity lifecyclePanel"><div className="panelHead"><span>CASE LIFECYCLE</span><small>PERSISTED TIMELINE</small></div><div className="lifecycleRail">{lifecycle.map((step,index)=><div className={`${step.done?"done":"pending"} ${step.tone}`} key={step.label}><i>{step.done?"✓":index+1}</i><b>{step.label}</b><span>{step.detail}</span></div>)}</div><div className="now"><i/><div><b>{trace?.timeline.at(-1)?.message||"Tracing funds"}</b><span>Every event below is persisted; external enrichment is visually marked and never promoted to chain evidence.</span></div></div><div className="timeline">{(trace?.timeline||[]).slice().reverse().map(e=>{const telegraph=e.type.includes("TELEGRAPH");const movement=["MOVEMENT_DETECTED","TRACING_RESUMED","BRANCH_DORMANT","MONITORING_ACTIVE"].includes(e.type);return <div className={telegraph?"eventTelegraph":movement?"eventMovement":""} key={e.id}><time>{new Date(e.created_at).toLocaleTimeString()}</time><i/><p><b>{e.message}</b><span>{telegraph?"TELEGRAPH · EXTERNAL":e.type.replaceAll("_"," ")}</span></p></div>})}</div></section>}
{section==="evidence"&&<section className="panel diagnosis"><div className="panelHead"><span>INCIDENT SELECTION</span><small>{c.discovery?`${Math.round(c.discovery.incident_selection_confidence*100)}% CONFIDENCE`:"USER SUPPLIED"}</small></div><h3>{c.discovery?"Incident identified from wallet history":"Transaction supplied by investigator"}</h3><p>{c.discovery?"NEMESIS reviewed this wallet's outgoing history and identified this transaction as the incident, then verified it directly on the blockchain.":"This transaction was independently verified on the blockchain."}</p>{c.discovery?.candidates[0]?.reasons.length?<details className="techDetails"><summary>Technical details</summary><ul className="reasonList">{c.discovery.candidates[0].reasons.map(r=><li key={r}>{r}</li>)}</ul></details>:null}</section>}
  {section==="assessment"&&<section className="panel diagnosis agentPlane"><div className="panelHead"><div><span>AGENT ASSESSMENT</span><small> INTERPRETATION · NOT EVIDENCE</small></div><small>WHAT THIS EVIDENCE MEANS</small></div><h3>Compromise method</h3>{undetermined?<><p className="assessHeadline">Not yet determined</p><p>There is not enough verified evidence yet to determine how this wallet was compromised. The transaction, the assets that moved and where they went are confirmed; the method behind the compromise is not, and NEMESIS will not guess at it.</p></>:<><p className="assessHeadline">{f.classification.replaceAll("_"," ")}</p><p>{f.summary}</p></>}<div className="evidenceRows"><span>Submitted wallet <b>{short(c.wallet_address)}</b></span><span>Transaction <b>{short(tx.hash)}</b></span><span>From <b>{short(tx.from_address)}</b></span><span>To <b>{short(tx.to_address)}</b></span>{c.discovery&&<span>Incident discovery <b>{c.discovery.source.toUpperCase()} · {c.discovery.candidate_count} candidates</b></span>}</div>{f.limitations.length>0&&<small className="caution">{f.limitations.join(" · ")}</small>}<details className="techDetails"><summary>Technical details</summary><dl className="tgTechnical"><div><dt>Classification</dt><dd>{f.classification}</dd></div><div><dt>Model confidence</dt><dd>{Math.round(f.confidence*100)}%</dd></div>{f.compromise_mechanism_confidence!==null&&<div><dt>Mechanism confidence</dt><dd>{Math.round(f.compromise_mechanism_confidence*100)}%</dd></div>}{f.evidence_references.length>0&&<div><dt>Evidence references</dt><dd>{f.evidence_references.join(", ")}</dd></div>}</dl></details></section>}
  {section==="graph"&&<section className="panel destination"><div className="panelHead"><span>TRACE BRANCHES</span><small>{trace?.branches.length||0} FOUND</small></div>{trace?.branches.length?<div className="transferList">{trace.branches.map(b=><div key={b.id}><span>{b.status}</span><b>{short(b.current_address)}</b><small>{short(b.asset)} · raw amount {b.amount}</small></div>)}</div>:<div className="emptyTrace">No qualifying outgoing fund path was present.</div>}</section>}
  {section==="evidence"&&trace?.outcome&&<section className="panel destination"><div className="panelHead"><span>CURRENT OUTCOME</span><small>DETERMINISTIC STATE</small></div><h3>{trace.outcome.summary}</h3><div className="transferList">{trace.outcome.asset_totals.map(total=><div key={total.asset}><span>{total.asset==="native"?"NATIVE ETH":short(total.asset)}</span><b>Stolen {amount(total.stolen,total.asset)} · Located {amount(total.located,total.asset)} · Unresolved {amount(total.unresolved,total.asset)}</b><small>exact units: {total.stolen} stolen · {total.located} located</small></div>)}</div><div className="evidenceRows">{Object.entries(trace.outcome.branch_counts).filter(([,count])=>count>0).map(([status,count])=><span key={status}>{status.toUpperCase()} <b>{count}</b></span>)}</div><h3>What you can do now</h3><p>{trace.outcome.next_actions.join(" · ")}</p><small className="caution">{trace.outcome.limitations.join(" · ")}</small></section>}
  {section==="intelligence"&&<TelegraphPanel state={telegraph} loading={tgLoading}/>}
  {section==="evidence"&&<section className="panel escalation chainPlane"><div className="panelHead"><div><span>ONCHAIN EVIDENCE</span><small> AUTHORITATIVE · VERIFIED ONCHAIN</small></div><small>STORED</small></div><div className="packageIcon">▤</div><div><h3>Normalized transaction package</h3><p>The transaction, the assets that moved and where they went are confirmed directly on the blockchain. External intelligence and the agent assessment can add context, but neither can change these facts.</p></div>{isPublic?<p className="publicNote">Public demo mode is read-only. The complete evidence package remains protected with the investigation account.</p>:<div className="evidenceActions"><button onClick={()=>exportEvidence("report")}>Download incident report</button><button className="secondary" onClick={()=>exportEvidence("package")}>Evidence package (JSON)</button></div>}</section>}
 </div></section>}</Shell>
}
...
export default function Home(){
 ...
 // Ambiguity no longer stops an investigation; this remains only for a case
 // that genuinely produced no verifiable candidate to continue from.
 ...
 if(view==="real"&&real)return <RealCaseScreen data={real} isPublic={publicCase} onReplace={setReal} onExit={()=>{setPublicCase(false);setView(publicCase?"landing":"intake")}} onHome={()=>{setPublicCase(false);setView("landing")}}/>;
